"""
est.xlsx 단가산출 시트 파서 v3 (호표 사이 모든 데이터 수집)
...
def read_sangcul_list(file_path):
    """단가산출총괄표 읽기"""
    try:
        df = pd.read_excel(file_path, sheet_name='단가산출총괄표', header=None)
...
def extract_detailed_content(df, start_row, end_row):
    ...
    for row in range(start_row, end_row):
        ...
        # 모든 컬럼 검사 (est는 col 1부터 시작)
        for col in range(1, min(16, len(df.columns))):
        ...
        if has_content:
            main_content = row_data['columns'].get('col_1', '')
            ...
            # 비용 항목
                ...
                for col_key, value in row_data['columns'].items():
                    if col_key != 'col_1' and re.match(r'[\d,\.]+', str(value)):
...
def parse_est_sangcul():
    """est.xlsx의 단가산출 시트 파싱 (세부 데이터 완전 수집)"""
    ...
    try:
        ...
        # JSON 파일로 저장
        output_file = 'est_sangcul_parsed_v3.json'
